function [out,grad,Nph_max_fact] = CostPR_gpu_v2(phase_mask,IS,q,data,std_stack,Nph_vec,cost_function_flag...
    ,gBlur ,k,opt_phase_mat,g_bfp_init,vec_model_flag,Nph_opt_flag,cost_altr,vec_model_pol,pmask_perfect)
...
%         nom = ((tmp+std_Gbg.^2)-2*(tmp+std_Gbg.^2).*(data_z0-tmp)-(data_z0-tmp).^2).*(data_z0~=0);
   ...
   elseif cost_function_flag==5
       gamma=2;
        % double gaussian MLE
        cost(z_ind) = sum((log(sqrt(2*pi*(tmp(:)+std_Gbg.^2+eps)))+0.5*((data_z0(:)-tmp(:)).^2)./(tmp(:)+std_Gbg.^2)).*(data_z0(:)~=0))+gamma*sum((phase_mask-pmask_perfect).^2,'all');
        % derivative
        denom = 2*(tmp+std_Gbg.^2+eps).^2+gamma*2*(phase_mask-pmask_perfect);
%         nom = ((tmp+std_Gbg.^2)-2*(tmp+std_Gbg.^2).*(data_z0-tmp)-(data_z0-tmp).^2).*(data_z0~=0);
        nom = ((tmp+std_Gbg.^2)-2*(tmp+std_Gbg.^2).*(data_z0-tmp)-(data_z0-tmp).^2);
        dcost_dtmp = nom./denom;
