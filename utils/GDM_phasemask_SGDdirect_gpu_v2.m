...
while iter<=maxiter
    ...
    elseif sample_flag ==3 % at worst corr2
        ...
        for z_ind = 1:length(grid_sampling)
            ...
            tmp = gather(tmp(round(end/2-IS.FOV_size/2)+1:round(end/2+IS.FOV_size/2),round(end/2-IS.FOV_size/2)+1:round(end/2+IS.FOV_size/2)));
            %crop
    ...
    if  mod(iter,30) == 1 && plot_flag
        ...
        for z_ind = 1:length(ind_opt)
            ...
            % plots
            subplot(2,length(ind_opt),countS);
            countS=countS+1;
    ...
    if (est_gBlur_flag>0 && (iter == floor(IS.SGDiter/3)))
        ...
        for j = 1:size(data,3)
            ...
            tmp = gather(Nph(j)*(GenFunc(x,q(j,:))));
            model_stack(:,:,j) = gather(tmp(round(end/2-IS.FOV_size/2)+1:round(end/2+IS.FOV_size/2),round(end/2-IS.FOV_size/2)+1:round(end/2+IS.FOV_size/2)));
            % crop data
        ...
        CostgBlur(gB,data_stack,model_stack,IS.gBlur_cost,gather(std_stack0),(thr_dat));
